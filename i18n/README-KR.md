...
- 제품 정보로 검색용 질의를 만들고 `gemini-embedding-001`과 코사인 KNN으로 구조화된 구성
  양식을 검색합니다.
- 의미 점수에 같은 분류 가중치를 더해 재사용 가능한 세 개의 설득 목적 구성 양식 중 하나를
  선택합니다.
- 승인된 분류와 제품 유형으로 제품군 모듈을 결정하고, 공통 4단계 흐름에 적용 가능한 제품군
  영역을 조합합니다.
- 조합된 영역 구조를 Gemini가 채우며, 검토를 위해 입력 출처 필드를 함께 보존합니다.

### 계획 기반 이미지와 게시 가능 HTML

- 승인된 제품 사실을 로봇청소기 8개 능력군으로 정규화합니다.
- 제품군 모듈의 정확한 영역 연결과 미디어 프로필을 사용해 최대 8개의 근거 있는 이미지
  슬롯을 동적으로 계획합니다.
...
flowchart TB
    ...
    E --> NF["MediaFacts 의미 정규화"]
    E --> K["Gemini 임베딩<br/>KNN + 분류 가중치"]
    K --> PT["설득 목적 구성 양식<br/>공통 4단계"]
    NF --> CM["제품군 모듈<br/>조건부 영역"]
    PT --> CT["조합된 스토리 영역"]
    CM --> CT
    CT --> G["Gemini 구조화 본문"]
    G --> V["스키마·입력 근거 검증"]
    CM --> MP["미디어 프로필 + 정확한 영역 연결<br/>동적 MediaPlan"]
    NF --> MP
