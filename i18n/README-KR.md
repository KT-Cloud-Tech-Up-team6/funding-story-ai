...
[사용 예시](#-포함된-예시-합성-로봇청소기) · [처리 구조](#-처리-구조) ·
...
```mermaid
flowchart TB
    U["사용자 대화·제품 이미지"] --> A["대화 처리기<br/>정보 추출·추가 질문·생성 확인"]
...
이 저장소는 공개 크라우드펀딩 자료와 관찰 가능한 Story AI 동작을 참고한 독립
구현입니다. 와디즈와 공식 제휴·승인·연동된 프로젝트가 아니며, 비공개 지시문·API·
모델·구성 양식 원본이나 외부 서비스의 원본 실행 결과를 포함하지 않습니다.

언어별 README 배치와 언어 선택 표시는
[Flowise](https://github.com/FlowiseAI/Flowise)의 방식을 참고했습니다. 정보 구성은
[Outlines](https://github.com/dottxt-ai/outlines)의
`Overview → Quick Start → Use cases → Architecture` 흐름을 참고했으며, 문구·코드·예시는
이 저장소에 맞게 작성했습니다.
