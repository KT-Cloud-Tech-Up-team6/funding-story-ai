...
def build_story_prompt(
    *,
    ...
    return f"""당신은 한국어 크라우드펀딩 스토리 초안을 작성합니다.
...
7. 제품군에서 흔하다는 이유로 자동 복귀, 물통 보충·비움 방식, 전용 앱 여부처럼
   입력에 없는 동작·관리 방식·소유 관계를 보완하지 마세요.
8. 유사한 표현이라도 의미를 확대하지 마세요. 예를 들어 장애물 감지를 경로 감지로,
   앱 경험을 앱 연동 경험으로 바꾸지 마세요.
...
11. `자동`, `완전`, `필요 없음`, `방지`, `해결`처럼 동작 또는 효과를 강화하는 표현은
    브리프에 같은 의미가 명시된 경우에만 사용하세요. 예를 들어 `먼지 비움`을
    `자동 먼지 비움`으로 확장하지 마세요.
