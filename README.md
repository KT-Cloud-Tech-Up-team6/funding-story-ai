...
This repository is an independent implementation informed by public crowdfunding materials and
observable Story AI behavior. It is not affiliated with, endorsed by, or integrated with Wadiz. It
contains no private prompts, APIs, models, original templates, or original external-service outputs.

The bilingual README placement and language selector follow the pattern used by
[Flowise](https://github.com/FlowiseAI/Flowise). The information flow is informed by the
`Overview → Quick Start → Use cases → Architecture` progression in
[Outlines](https://github.com/dottxt-ai/outlines). All wording, code, and examples are specific to
this repository.
